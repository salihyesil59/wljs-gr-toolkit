...
This is the last link, and with it every $G_{\text{eff}}$ the series uses is derived inside it.
::*)
...
(*::md::
## 6. The Euler-Lagrange operator

The same trap GR-06 documents applies here, so the same fix is used. The obvious Wolfram
spelling of the field-theory Euler-Lagrange operator silently discards the spatial term, which
is exactly the one carrying $k^2$. The toy Lagrangian below is the one-line test that catches
it; the single-variable operator used for the background above is unaffected.
::*)
...
Module[{ph, trunc1, gpert, uUp, rhoT, tUp, sqg, bgMet, b00, b11},
 ...
 bgMet = Association[# -> EulerLagrange2D[lagQ1, #] & /@ metVars];
...
Module[{ph, subst, onShell, gMet, gCon},
 ...
 gMet = Association[# -> subst[EulerLagrange2D[lagQ2, #]] & /@ metVars];
 gCon = Association[# -> subst[EulerLagrange2D[lagQ2, #]] & /@ conVars];
