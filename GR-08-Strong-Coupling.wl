...
- **In $f(Q)$ the bare kinetic matrix cannot settle the question**, and section 8 says why
  rather than pretending otherwise. GR-09 settles it properly, with an Ostrogradsky reduction
  and a Dirac constraint analysis, and finds one propagating scalar mode.
...
**GR-09 does attempt it, and the answer is one propagating scalar mode.** The constraint count
is the same twelve in general $f(Q)$ as in general relativity; what changes is that two
constraints move from first class to second, which is a broken gauge symmetry leaving half of
its pair behind as a physical mode. Rank four here was a symptom of the second-derivative
problem, not a mode count: at STEGR the correct analysis returns zero, as it must.
...
of the connection modes is open *at this level of analysis*, and GR-09 closes it: one of them
propagates, on a de Sitter background, in the scalar sector.
...
Dataset @ {
  ...
  <|"theory" -> "f(Q)",  "extra modes" -> "two connection scalars, one propagates (GR-09)",
    "c_GW^2" -> "1", "linear theory reliable" -> "metric sector only"|>}
