...
Nothing in this library is quoted any more. The $f(R)$ forms are derived in **GR-05**, which
perturbs the field equations in Newtonian gauge and solves for the metric potentials. The
$f(\mathcal{T})$ form is derived in **GR-06**, by varying all sixteen components of a perturbed
tetrad in a second order action. The $f(Q)$ form is derived in **GR-07**, by carrying the flat
connection as four Stückelberg fields and varying those alongside the metric.

The pattern that comes out is worth keeping in mind while choosing an entry. $f(R)$ carries a
scalaron with a Compton wavelength, so its $G_{\text{eff}}$ has a scale to compare $k$ against
and runs by a factor of $4/3$ between large and small scales, with a slip that goes to $1/2$.
Neither teleparallel family has such a scale: both give a pure rescaling,
$G_{\text{eff}}/G = 1/f_{\mathcal{T}}$ and $1/f_Q$, scale independent and with no slip at all.
A survey covering a range of scales can see that difference.
