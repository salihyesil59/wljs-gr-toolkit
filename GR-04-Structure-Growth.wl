...
by integration. $G_{\text{eff}}/G$ is **input**, not output. In general relativity it is
exactly 1. For modified gravity the expressions in the library are the standard quasi-static,
sub-horizon results quoted from the literature; deriving them means perturbing the field
equations of each theory, which is a longer job than this notebook does. Check the convention
against your own source before trusting a number, and note that in $f(R)$ gravity
$G_{\text{eff}}$ depends on the wavenumber $k$ while in $f(\mathcal{T})$ and $f(Q)$ it does not
— that scale dependence is itself a discriminator.
