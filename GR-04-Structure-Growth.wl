...
Where the modified-gravity entries come from splits three ways now. The $f(R)$ forms are
derived from scratch in **GR-05**, which perturbs the field equations in Newtonian gauge and
solves for the metric potentials. The $f(\mathcal{T})$ form is derived in **GR-06**, by varying
all sixteen components of a perturbed tetrad in a second order action; it comes out as
$G_{\text{eff}}/G = 1/f_{\mathcal{T}}$ in the sign convention used here, scale independent and
with no gravitational slip. Only the $f(Q)$ entry is still quoted from the literature, because
its fundamental variable is a flat non-metric connection that neither of those calculations
reaches. Check the convention against your own source for that one.
